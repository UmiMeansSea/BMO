...
# Section 5
doc.add_heading('5. Empirical Evaluation: A Multi-Generational Green AI Benchmark', level=1)
doc.add_paragraph(
    "To evaluate BMO under Green AI principles across architectural generations, we measure Floating Point Operations (FPO). FPO provides a hardware-agnostic estimate of computational work, directly correlating to energy consumption. The analytical FPO for an inference turn is calculated as:\n\n"
    "FPO = 2 * P * T\n\n"
    "Where P represents the active parameter count (in billions) and T is the total number of processed and generated tokens. By utilizing Dense Micro-CoT, T drops significantly, maintaining extreme energy efficiency even when scaling parameter capacity.\n\n"
    "We benchmarked BMO across 4 architectural eras on local CPU hardware using a 50-sentence blind holdout set containing complex French morphological rules."
)

# Table
table_data = [
    ["Architectural Era", "Parameters (P)", "Work per Turn", "CPU Latency", "Carbon (200 Turns)", "Pass Rate"],
    ["Era 1: 1.5B (Zero-Shot)", "1.5 B", "150.0 B FPO", "3.10 s", "19.61 g CO2e", "40.0%"],
    ["Era 2: 3B (Baseline)", "3.4 B", "340.0 B FPO", "10.40 s", "32.10 g CO2e", "20.0%"],
    ["Era 3: BMO 3B (Micro-CoT)", "3.4 B", "170.0 B FPO", "3.53 s", "7.10 g CO2e", "76.0%"],
    ["Era 4: BMO 7B (Micro-CoT)", "7.6 B", "380.0 B FPO", "8.41 s", "15.15 g CO2e", "76.0%"],
    ["Cloud API Equivalent", "~70.0 B", "8,400.0 B FPO", "2.50 s (Net)", "343.14 g CO2e", "88.0%"]
]
...
analysis_bullets = [
    "ChatML Index 0 Alignment: Restoring system instructions explicitly at index 0 eliminated parroting and conversational looping, aligning model reasoning across both 3B and 7B scales.",
    "Optimal Edge Operating Point: BMO 3B Micro-CoT achieves the optimal edge trade-off (76.0% accuracy at 3.53s CPU latency), while BMO 7B Micro-CoT provides enhanced vocabulary depth at 8.41s CPU latency.",
    "Decoupling Accuracy from Carbon Footprint: By employing Dense Micro-CoT reasoning (~15 tokens), BMO reduces computational work by up to 98% compared to cloud APIs while preserving high pedagogical fidelity."
]
